...
void MqttClient::start()
{
    try
    {
        ...
        if(_clientId.isEmpty()) {
            // If no client ID is specified, look for one in the parameters
            if(_parameters.clientId().isEmpty()) {
                // If no client ID is specified in parameters, use a generated UUID
                _client->setClientId(QUuid::createUuid().toString(QUuid::WithoutBraces));
            }
            else {
                _client->setClientId(_parameters.clientId());
            }
        }
        else {
        ...
        if(_parameters.useSsl()) {
            ...
            if(_parameters.clientCertificate().isNull() != _parameters.privateKey().isNull()) {
                throw CommonException("If client cert is set, the private key must also be set (and vice versa)");
            }

            if(_parameters.clientCertificate().isNull() == false) {
                _sslConfig.setLocalCertificate(_parameters.clientCertificate());
            }
...
void MqttClient::onErrorChanged(QMqttClient::ClientError errorCode)
{
    logText(LVL_DEBUG, QString("%1: Error changed to %2").arg(objectName()).arg(errorCode));
    error(errorCode);
