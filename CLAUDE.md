...
- **Client-certificate support is built into the base.** `setLocalCertificate(cert)` / `setLocalCertificateChain(chain)` plus `setPrivateKey(key)` configure mutual-TLS directly on any `HttpOperation` — no subclassing required. Use `setLocalCertificateChain()` (leaf first, followed by its issuing intermediate CA(s)) when the peer's trust store holds only a root CA and needs the intermediate(s) to build the path. `setVerifyPeer(false)` and `ignoreSelfSignedCertificate()` remain the peer-side TLS knobs. **Do not override `configureSsl(QNetworkRequest*)` to hand-roll mTLS** — the base implementation carries the leaf-validity gate that stops a null or malformed leaf from engaging mTLS, and an override bypasses it. `MqttClient` (via `MqttParameters`) does support client certs directly.
...
- **`HttpOperation` base supports client certificates directly.** TAU/SGW REST integration that needs mutual TLS calls `setLocalCertificate()` / `setLocalCertificateChain()` plus `setPrivateKey()` on the operation — no subclassing needed. `setLocalCertificateChain()` takes a leaf-first chain (the client's own certificate followed by its issuing intermediate CA(s)) for peers whose trust store holds only a root CA. **Do not override `configureSsl(QNetworkRequest*)` to hand-roll mTLS** — the base implementation carries the leaf-validity gate that stops a null or malformed leaf from engaging mTLS, and an override bypasses it. If you need this in new code, look at how `CloudGet` / `CloudPost` are built.
